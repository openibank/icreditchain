...
use creditchain_framework::{extended_checks, path_in_crate, BuildOptions};
use creditchain_gas_schedule::{MiscGasParameters, NativeGasParameters, LATEST_GAS_FEATURE_VERSION};
use creditchain_types::on_chain_config::{
    creditchain_test_feature_flags_genesis, Features, TimedFeaturesBuilder,
};
use creditchain_vm::natives;
...
/// TODO: per @vgao1996:
/// - There should be only one ground truth of `creditchain_test_natives`.
...
/// More discussion: https://github.com/ibankio/creditchain/pull/15997#discussion_r1994469668
pub fn creditchain_test_natives() -> NativeFunctionTable {
...
fn move_framework_unit_tests() {
    run_tests_for_pkg("creditchain-framework", false);
}

#[test]
fn move_creditchain_stdlib_unit_tests() {
    run_tests_for_pkg("creditchain-stdlib", false);
}
...
fn move_token_unit_tests() {
    run_tests_for_pkg("creditchain-token", false);
}

#[test]
fn move_token_objects_unit_tests() {
    run_tests_for_pkg("creditchain-token-objects", false);
}

#[test]
fn move_experimental_unit_tests() {
    run_tests_for_pkg("creditchain-experimental", true);
}
