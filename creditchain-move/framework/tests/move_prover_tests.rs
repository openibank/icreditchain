...
use creditchain_framework::{extended_checks, prover::ProverOptions};
...
fn move_framework_prover_tests() {
    run_prover_for_pkg("creditchain-framework", 1, None);
}

#[test]
fn move_token_prover_tests() {
    run_prover_for_pkg("creditchain-token", 1, None);
}

#[test]
fn move_creditchain_stdlib_prover_tests() {
    run_prover_for_pkg("creditchain-stdlib", 1, None);
}
