/// ---
/// title: "Integrate with CreditChain"
...
///
/// # Integrate with the CreditChain Blockchain
///
/// If you provide blockchain services to your customers and wish to add the CreditChain blockchain to your platform, then this guide is for you. This system integrators guide will walk you through all you need to integrate the CreditChain blockchain into your platform.
///
/// ## Overview
///
/// This document will guide you through the following tasks to integrate with CreditChain:
...
/// ### SDKs and tools
///
/// CreditChain currently provides three SDKs:
...
///
/// CreditChain supports two methods for constructing transactions:
///
/// - Using the CreditChain client libraries to generate native BCS transactions.
...
///
/// Most integrations into the CreditChain blockchain benefit from a holistic and comprehensive overview of the current and historical state of the blockchain. CreditChain provides historical transactions, state, and events, all the result of transaction execution.
///
...
///
/// The current balance for a `Coin<T>` where `T` is the CreditChain coin is available at the account resources URL: `https://{rest_api_server}/accounts/{address}/resource/0x1::coin::CoinStore<0x1::creditchain_coin::CreditChainCoin>`. The balance is stored within `coin::amount`. The resource also contains the total number of deposit and withdraw events, and the `counter` value within `deposit_events` and `withdraw_events`, respectively.
///
/// ```
/// {
///   "type": "0x1::coin::CoinStore<0x1::creditchain_coin::CreditChainCoin>",
///   "data": {
///     "coin": {
///       "value": "3927"
///     },
...
///
/// In CreditChain, each transaction is committed as a distinct version to the blockchain. This allows for the convenience of sharing committed transactions by their version number; to do so, query: `https://{rest_server_api}/transactions/by_version/{version}`
///
...
/// :::tip
///
/// When tracking full movement of coins, normally events are sufficient. `0x1::creditchain_coin::CreditChainCoin`, however, requires considering `gas_used` for each transaction sent from the given account since it represents gas in CreditChain. To reduce unnecessary overhead, extracting gas fees due to transactions does not emit an event. All transactions for an account can be retrieved from this API: `https://{rest_server_api}/accounts/{address}/transactions`
///
/// :::
///
...
/// 3. A gas fee `2200` of `0x1::creditchain_coin::CreditChainCoin` from the sending account `0x810026ca8291dd88b5b30a1d3ca2edd683d33d06c4a7f7c451d96f6d47bc5e8b`
///
...
/// 2. Gas is always: `0x1::creditchain_coin::CreditChainCoin`
///
