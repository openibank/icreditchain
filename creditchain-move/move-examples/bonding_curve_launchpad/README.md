...
Once the LBT threshold is met within the liquidity pair, the reserves are moved onto a public DEX, referred to as graduation. From there, the global freeze is disabled, allowing all participants to freely use their tokens.
...
**Virtual Liquidity** - To prevent early adopter's advantage, a pre-defined amount of virtual liquidity is assumed to exist in all LBT reserves for liquidity pairs. Since the FA and LBT reserves will be closer together in value, an early transfer won't be as dramatic for the number of tokens received. 
...
* E2E tests.
### `move-examples/swap`
* Fungible Asset DEX example from the `creditchain-move` examples.
...
2. `bonding_curve_launchpad` creates a liquidity pair on the `liquidity_pairs` module, which follows the constant product formula.
   ...
   2. The entirety of minted FA + a pre-defined number of **virtual liquidity** for LBT is deposited into the liquidity pair object. 
...
3. Optionally, the creator can immediately initiate a swap from LBT to the FA.
#### Trading against the FA's associated liquidity pair
1. External users can swap LBT to FA, or vice versa, through `public entry` methods available on `bonding_curve_launchpad`. 
...
1. After each swap from LBT to FA, when the LBT reserves are increasing, a threshold is checked for whether the liquidity pair can **graduate** or not. The threshold is a pre-defined minimum amount of LBT that must exist in the reserves. Once this threshold is met during a swap, **graduation** begins.
   ...
   2. The reserves from the liquidity pair, both LBT and FA, are moved to an external, public third-party DEX as a new liquidity pair. In this case, the `creditchain-move` FA DEX example, called swap. 
