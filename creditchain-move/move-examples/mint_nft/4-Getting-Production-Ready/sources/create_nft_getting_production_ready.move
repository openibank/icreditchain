...
/// We use signature validation for many reasons. For example, in creditchain_framework::account::rotate_authentication_key(),
...
/// For more information about events, see: https://docs.creditchain.org/concepts/events/.
///
/// Move unit tests
/// We added a few unit tests to make sure that our code is working as expected. For more information on how to write
/// Move unit tests, see: https://docs.creditchain.org/move/book/unit-testing
///
...
/// Run the following command in directory `creditchain-core/creditchain-move/move-examples/mint_nft/4-Getting-Production-Ready`.
...
///  Open up file `creditchain-core/creditchain-move/e2e-move-tests/src/tests/mint_nft.rs`.
...
///  You can find the `admin_private_key` by running `nano output.key`, and the `receiver_account_sequence_number` by looking up the receiver's address on the CreditChain Explorer under tab `Info`.
///  Make sure you're in the right directory.
///  Run the following command in directory `creditchain-core/creditchain-move/e2e-move-tests`
...
module mint_nft::create_nft_getting_production_ready {
    ...
    use creditchain_token::token::{Self, TokenDataId};
    use creditchain_framework::resource_account;
    #[test_only]
    use creditchain_framework::account::create_account_for_test;
    use creditchain_std::ed25519::ValidatedPublicKey;
    ...
    public entry fun test_happy_path(
        ...
        set_up_test(origin_account, &resource_account, &admin_pk, creditchain_framework, &nft_receiver, 10);
    ...
    public entry fun test_minting_expired(
        ...
        let (admin_sk, admin_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &resource_account, &admin_pk, creditchain_framework, &nft_receiver, 100000000001);
        let receiver_addr = signer::address_of(&nft_receiver);
        let proof_challenge = MintProofChallenge {
            receiver_account_sequence_number: account::get_sequence_number(receiver_addr),
            receiver_account_address: receiver_addr,
