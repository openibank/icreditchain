...
const DEVTOOL_TARGET_DIRECTORY: &str = "target/creditchain-x-tool";

// File types in `creditchain-core` that are not relevant to the rust build and test process.
...
// Paths in `creditchain-core` that are not relevant to the rust build and test process.
// Note: this is a best effort list and will need to be updated as time goes on.
...
fn read_metadata_from_gcs(
    ...
    let url = format!(
        "https://storage.googleapis.com/creditchain-core-cargo-metadata-public/{}",
        remote_file_name
    );
