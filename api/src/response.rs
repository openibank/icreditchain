...
//! The CreditChain API response / error handling philosophy.
//!
...
pub enum CreditChainResponseContent<T: ToJSON + Send + Sync> {
...
/// an error response to manipulate the CreditChainError inside it.
pub trait CreditChainErrorResponse {
    fn inner_mut(&mut self) -> &mut CreditChainError;
}
...
/// implementor also implements CreditChainErrorResponse, which saves functions from
...
macro_rules! generate_error_traits {
    ($($trait_name:ident),*) => {
        paste::paste! {
        $(
        pub trait [<$trait_name Error>]: CreditChainErrorResponse {
            // With ledger info and an error code
            #[allow(unused)]
            fn [<$trait_name:snake _with_code>]<Err: std::fmt::Display>(
                err: Err,
                error_code: creditchain_api_types::CreditChainErrorCode,
                ledger_info: &creditchain_api_types::LedgerInfo,
            ...
            fn [<$trait_name:snake _with_optional_vm_status_and_ledger_info>]<Err: std::fmt::Display>(
                err: Err,
                error_code: creditchain_api_types::CreditChainErrorCode,
                vm_status: Option<creditchain_types::vm_status::StatusCode>,
                ledger_info: Option<&creditchain_api_types::LedgerInfo>
            ) -> Self where Self: Sized;
            ...
            fn [<$trait_name:snake _with_vm_status>]<Err: std::fmt::Display>(
                err: Err,
                error_code: creditchain_api_types::CreditChainErrorCode,
                vm_status: creditchain_types::vm_status::StatusCode,
                ledger_info: &creditchain_api_types::LedgerInfo
            ) -> Self where Self: Sized;

            #[allow(unused)]
            fn [<$trait_name:snake _from_creditchain_error>](
                creditchain_error: creditchain_api_types::CreditChainError,
                ledger_info: &creditchain_api_types::LedgerInfo
            ) -> Self where Self: Sized;
        }
        )*
...
macro_rules! generate_error_response {
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name {
            $(
            #[oai(status = $status)]
            $name(poem_openapi::payload::Json<Box<creditchain_api_types::CreditChainError>>,
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                /// Chain ID of the current chain
                #[oai(header = "X-CreditChain-Chain-Id")] Option<u8>,
                /// Current ledger version of the chain
                #[oai(header = "X-creditchain-ledger-Version")] Option<u64>,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-creditchain-ledger-Oldest-Version")] Option<u64>,
                /// Current timestamp of the chain
                #[oai(header = "X-creditchain-ledger-TimestampUsec")] Option<u64>,
                /// Current epoch of the chain
                #[oai(header = "X-CreditChain-Epoch")] Option<u64>,
                /// Current block height of the chain
                #[oai(header = "X-CreditChain-Block-Height")] Option<u64>,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-CreditChain-Oldest-Block-Height")] Option<u64>,
                /// The cost of the call in terms of gas
                #[oai(header = "X-CreditChain-Gas-Used")] Option<u64>,
            ),
            )*
        }
        ...
        impl $crate::response::[<$name Error>] for $enum_name {
            fn [<$name:snake _with_code>]<Err: std::fmt::Display>(
                ...
                let error = creditchain_api_types::CreditChainError::new_with_error_code(err, error_code);
            ...
            fn [<$name:snake _with_optional_vm_status_and_ledger_info>]<Err: std::fmt::Display>(
                ...
                let error = if let Some(vm_status) = vm_status {
                    creditchain_api_types::CreditChainError::new_with_vm_status(err, error_code, vm_status)
                } else {
                    creditchain_api_types::CreditChainError::new_with_error_code(err, error_code)
                };
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
            ...
            fn [<$name:snake _with_vm_status>]<Err: std::fmt::Display>(
                ...
                let error = creditchain_api_types::CreditChainError::new_with_vm_status(err, error_code, vm_status);
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
            ...
            fn [<$name:snake _from_creditchain_error>](
                creditchain_error: creditchain_api_types::CreditChainError,
                ledger_info: &creditchain_api_types::LedgerInfo
            ) -> Self where Self: Sized {
                let payload = poem_openapi::payload::Json(Box::new(creditchain_error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
                    Some(ledger_info.ledger_version.into()),
        ...
        // Generate a function that helps get the CreditChainError within.
        impl $crate::response::CreditChainErrorResponse for $enum_name {
            fn inner_mut(&mut self) -> &mut creditchain_api_types::CreditChainError {
                match self {
                    $(
    ...
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name<T: poem_openapi::types::ToJSON + Send + Sync> {
            ...
            $name(
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                $crate::response::CreditChainResponseContent<T>,
                /// Chain ID of the current chain
                #[oai(header = "X-CreditChain-Chain-Id")] u8,
                /// Current ledger version of the chain
                #[oai(header = "X-creditchain-ledger-Version")] u64,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-creditchain-ledger-Oldest-Version")] u64,
                /// Current timestamp of the chain
                #[oai(header = "X-creditchain-ledger-TimestampUsec")] u64,
                /// Current epoch of the chain
                #[oai(header = "X-CreditChain-Epoch")] u64,
                /// Current block height of the chain
                #[oai(header = "X-CreditChain-Block-Height")] u64,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-CreditChain-Oldest-Block-Height")] u64,
                /// The cost of the call in terms of gas
                #[oai(header = "X-CreditChain-Gas-Used")] Option<u64>,
                ...
                #[oai(header = "X-CreditChain-Cursor")] Option<String>,
            ),
            )*
        }
        ...
        // Generate a From impl that builds a response from CreditChainResponseContent.
        ...
        impl <T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::response::CreditChainResponseContent<T>, &creditchain_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                    $crate::response::CreditChainResponseContent<T>,
                    &creditchain_api_types::LedgerInfo,
                    [<$enum_name Status>]
                ),
            ) -> Self {
                match status {
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<(poem_openapi::payload::Json<T>, &creditchain_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (poem_openapi::payload::Json<T>, &creditchain_api_types::LedgerInfo, [<$enum_name Status>]),
            ) -> Self {
                let content = $crate::response::CreditChainResponseContent::Json(value);
                Self::from((content, ledger_info, status))
            }
        }

        // Generate a From impl that builds a response from a Bcs<Vec<u8>> and friends.
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::bcs_payload::Bcs, &creditchain_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                ...
                let content = $crate::response::CreditChainResponseContent::Bcs(value);
                Self::from((content, ledger_info, status))
            }
        }
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync + serde::Serialize> $enum_name<T> {
                        ...
                        $crate::bcs_payload::Bcs(
                            bcs::to_bytes(&value)
                                .map_err(|e| E::internal_with_code(
                                    e,
                                    creditchain_api_types::CreditChainErrorCode::InternalError,
                                    ledger_info
                                ))?
                        ),
                        ledger_info,
           ...
           pub fn try_from_json<E: $crate::response::InternalError>(
            ...
            pub fn with_cursor(mut self, new_cursor: Option<creditchain_types::state_store::state_key::StateKey>) -> Self {
                match self {
                    $(
                    [<$enum_name>]::$name(_, _, _, _, _, _, _, _, _, ref mut cursor) => {
                        *cursor = new_cursor.map(|c| creditchain_api_types::StateKeyWrapper::from(c).to_string());
                    }
                    )*
                }
                self
...
pub fn json_api_disabled<S: Display, E: ForbiddenError>(identifier: S) -> E {
    E::forbidden_with_code_no_info(
        ...
        CreditChainErrorCode::ApiDisabled,
    )
}
...
pub fn version_not_found<E: NotFoundError>(ledger_version: u64, ledger_info: &LedgerInfo) -> E {
    build_not_found(
        ...
        CreditChainErrorCode::VersionNotFound,
        ledger_info,
    )
}
...
pub fn transaction_not_found_by_version<E: NotFoundError>(
    ...
    build_not_found(
        "Transaction",
        format!("Ledger version({})", ledger_version),
        CreditChainErrorCode::TransactionNotFound,
        ledger_info,
    )
}
...
pub fn version_pruned<E: GoneError>(ledger_version: u64, ledger_info: &LedgerInfo) -> E {
    E::gone_with_code(
        ...
        CreditChainErrorCode::VersionPruned,
        ledger_info,
    )
}
...
pub fn account_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        CreditChainErrorCode::AccountNotFound,
        ledger_info,
    )
}
...
pub fn module_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        CreditChainErrorCode::ModuleNotFound,
        ledger_info,
    )
}
...
pub fn struct_field_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        CreditChainErrorCode::StructFieldNotFound,
        ledger_info,
    )
}
...
pub fn table_item_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        CreditChainErrorCode::TableItemNotFound,
        ledger_info,
    )
}
...
pub fn block_not_found_by_height<E: NotFoundError>(
    ...
    build_not_found(
        ...
        CreditChainErrorCode::BlockNotFound,
        ledger_info,
    )
}
...
pub fn block_pruned_by_height<E: GoneError>(block_height: u64, ledger_info: &LedgerInfo) -> E {
    E::gone_with_code(
        ...
        CreditChainErrorCode::BlockPruned,
        ledger_info,
    )
}
