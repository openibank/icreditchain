...
| chain.max_stake | int | `100000000000000000` | Maximum stake. Defaults to 1B CREDITCHAIN coins with 8 decimals |
| chain.min_price_per_gas_unit | int | `1` | Minimum price per gas unit |
| chain.min_stake | int | `100000000000000` | Minimum stake. Defaults to 1M CREDITCHAIN coins with 8 decimals |
| chain.min_voting_threshold | int | `100000000000000` | Mininum voting threshold. Defaults to 1M CREDITCHAIN coins with 8 decimals |
...
| chain.required_proposer_stake | int | `100000000000000` | Required stake to be a proposer. 1M CREDITCHAIN coins with 8 decimals |
...
| genesis.fullnode.enable_onchain_discovery | bool | `true` | Use External DNS as created by creditchain-node helm chart for fullnode host in genesis |
...
| genesis.validator.enable_onchain_discovery | bool | `false` | Use External DNS as created by creditchain-node helm chart for validator host in genesis |
...
| genesis.validator.larger_stake_amount | string | `"1000000000000000"` | Stake amount for nodes we are giving larger state to. Defaults to 10M CREDITCHAIN coins with 8 decimals |
| genesis.validator.num_validators_with_larger_stake | int | `0` | Number of validators to give larger stake in genesis to. |
| genesis.validator.stake_amount | string | `"100000000000000"` | Stake amount for each validator in this testnet. Defaults to 1M CREDITCHAIN coins with 8 decimals |
