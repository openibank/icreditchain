...
#
# Runs an automated genesis ceremony for validators spun up by the creditchain-node helm chart
#
# Expect the following environment variables to be set before execution:
# NUM_VALIDATORS
# ERA
# WORKSPACE: default /tmp
# USERNAME_PREFIX: default creditchain-node
...
USERNAME_PREFIX=${USERNAME_PREFIX:-creditchain-node}
...
MOVE_FRAMEWORK_DIR=${MOVE_FRAMEWORK_DIR:-"/creditchain-framework/move"}
...
for i in $(seq 0 $(($NUM_VALIDATORS - 1))); do
  ...
  if [[ -z "${RANDOM_SEED}" ]]; then
    ...
    creditchain genesis generate-keys --random-seed $seed --output-dir $user_dir
  fi

  creditchain genesis set-validator-configuration --owner-public-identity-file $user_dir/public-keys.yaml --local-repository-dir $WORKSPACE \
    --username $username \
...
# run genesis
creditchain genesis generate-genesis --local-repository-dir ${WORKSPACE} --output-dir ${WORKSPACE}
