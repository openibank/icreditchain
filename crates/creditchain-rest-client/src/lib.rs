...
pub mod error;
pub mod faucet;
use error::CreditChainErrorResponse;
...
pub use crate::client_builder::{CreditChainBaseUrl, ClientBuilder};
use crate::{creditchain::CreditChainVersion, error::RestError};
use anyhow::{anyhow, Result};
pub use creditchain_api_types::{
...
use creditchain_api_types::{
    ...
    CreditChainError, CreditChainErrorCode, BcsBlock, Block, GasEstimation, HexEncodedBytes, IndexResponse,
...
use creditchain_logger::{debug, info, sample, sample::SampleRate};
...
const X_CREDITCHAIN_SDK_HEADER_VALUE: &str = concat!("creditchain-rust-sdk/", env!("CARGO_PKG_VERSION"));

type CreditChainResult<T> = Result<T, RestError>;
...
impl Client {
    pub fn builder(creditchain_base_url: CreditChainBaseUrl) -> ClientBuilder {
        ClientBuilder::new(creditchain_base_url)
    }

    pub fn new(base_url: Url) -> Self {
        Self::builder(CreditChainBaseUrl::Custom(base_url)).build()
    }
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> CreditChainResult<Self> {
    ...
    pub fn build_path(&self, path: &str) -> CreditChainResult<Url> {
    ...
    pub async fn get_creditchain_version(&self) -> CreditChainResult<Response<CreditChainVersion>> {
        self.get_resource::<CreditChainVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    ) -> CreditChainResult<Response<Block>> {
    ...
    pub async fn get_block_by_height_bcs(
        ...
        height: u64,
        with_transactions: bool,
    ) -> CreditChainResult<Response<BcsBlock>> {
    ...
    pub async fn lookup_address(
        ...
        must_exist: bool,
    ) -> CreditChainResult<Response<AccountAddress>> {
        ...
        {
            Ok(inner) => Ok(inner),
            Err(RestError::Api(CreditChainErrorResponse {
                error:
                    CreditChainError {
                        error_code: CreditChainErrorCode::TableItemNotFound,
                        ..
                    },
                ..
            })) => {
    ...
    pub async fn get_account_balance(
        ...
        asset_type: &str,
    ) -> CreditChainResult<Response<u64>> {
        let url = self.build_path(&format!(
            "accounts/{}/balance/{}",
            address.to_hex(),
            asset_type
    ...
    pub async fn view_apt_account_balance_at_version(
        ...
        self.view_account_balance_bcs_impl(address, "0x1::creditchain_coin::CreditChainCoin", Some(version))
            .await
    }
    ...
    pub async fn view_apt_account_balance(
        ...
        self.view_account_balance_bcs_impl(address, "0x1::creditchain_coin::CreditChainCoin", None)
            .await
    }

    pub async fn get_index(&self) -> CreditChainResult<Response<IndexResponse>> {
        self.get(self.build_path("")?).await
    }

    pub async fn get_index_bcs(&self) -> CreditChainResult<Response<IndexResponseBcs>> {
    ...
    pub async fn view(
        ...
        request: &ViewRequest,
        version: Option<u64>,
    ) -> CreditChainResult<Response<Vec<serde_json::Value>>> {
    ...
    ) -> CreditChainResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> CreditChainResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> CreditChainResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> CreditChainResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> CreditChainResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> CreditChainResult<Response<Transaction>> {
    ...
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = CreditChainResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await {
                ...
                Ok(WaitForTransactionResult::NotFound(error)) => {
                    if let RestError::Api(creditchain_error_response) = error {
                        if let Some(state) = creditchain_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if creditchain_infallible::duration_since_epoch().as_secs()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CreditChainResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CreditChainResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> CreditChainResult<Response<TransactionData>> {
    ...
    pub async fn get_transaction_by_hash_bcs_inner(
        &self,
        hash: HashValue,
    ) -> CreditChainResult<reqwest::Response> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> CreditChainResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> CreditChainResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> CreditChainResult<Response<Option<Resource>>> {
        let url = self.build_path(&format!(
            "accounts/{}/resource/{}",
            address.to_hex(),
            resource_type
    ...
    pub async fn get_account_resource_at_version_bytes(
        ...
        resource_type: &str,
        version: u64,
    ) -> CreditChainResult<Response<Vec<u8>>> {
        let url = self.build_path(&format!(
            "accounts/{}/resource/{}?ledger_version={}",
            address.to_hex(),
            resource_type,
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> CreditChainResult<Response<Vec<MoveModuleBytecode>>> {
        self.paginate_with_cursor(
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> CreditChainResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        self.paginate_with_cursor_bcs(
            &format!("accounts/{}/modules", address.to_hex()),
            MODULES_PER_CALL_PAGINATION,
            None,
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> CreditChainResult<Response<MoveModuleBytecode>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> CreditChainResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CreditChainResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_sequence_number(
        ...
        match res {
            ...
            Err(error) => match error {
                RestError::Api(error) => {
                    if matches!(error.error.error_code, CreditChainErrorCode::AccountNotFound) {
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CreditChainResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_table_item<K: Serialize>(
        ...
        value_type: &str,
        key: K,
    ) -> CreditChainResult<Response<Value>> {
    ...
    pub async fn get_account(&self, address: AccountAddress) -> CreditChainResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> CreditChainResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address.to_hex()))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }
    ...
    pub async fn get_account_transaction_summaries(
        ...
        limit: Option<u16>,
    ) -> CreditChainResult<Response<Vec<IndexedTransactionSummary>>> {
        let url = self.build_path(&format!(
    ...
    pub async fn estimate_gas_price(&self) -> CreditChainResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> CreditChainResult<String> {
    ...
    ) -> CreditChainResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> CreditChainResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> CreditChainResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> CreditChainResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
        self.check_and_parse_bcs_response(response).await
    }

    pub async fn make_prover_request(&self, req: ProverRequest) -> CreditChainResult<ZeroKnowledgeSig> {
    ...
    pub async fn make_pepper_request(&self, req: PepperRequest) -> CreditChainResult<Pepper> {
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<CreditChainError>) -> bool,
        Fut: Future<Output = CreditChainResult<T>>,
    {
    ...
    pub async fn paginate_with_cursor<T: for<'a> Deserialize<'a>>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> CreditChainResult<Response<Vec<T>>> {
    ...
    pub async fn paginate_with_cursor_bcs<T: for<'a> Deserialize<'a> + Ord>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> CreditChainResult<Response<BTreeMap<T, Vec<u8>>>> {
...
pub fn retriable_with_404(status_code: StatusCode, creditchain_error: Option<CreditChainError>) -> bool {
    retriable(status_code, creditchain_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _creditchain_error: Option<CreditChainError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> CreditChainResult<State> {
