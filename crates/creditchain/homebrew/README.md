...
software by checking upstream. The strategy used below checks for all `creditchain-cli-v<SEMVER>` tags for `creditchain`. The
...
...
  ...
  # with matching regex of href="<URL>/tag/creditchain-cli-v<SEMVER>". This
  # is done to automatically check for new release versions of the CLI.
  livecheck do
    url :stable
    regex(/^creditchain-cli[._-]v?(\d+(?:\.\d+)+)$/i)
  end
...
```
