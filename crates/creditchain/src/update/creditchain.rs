...
use creditchain_build_info::BUILD_OS;
...
impl BinaryUpdater for CreditChainUpdateTool {
    fn check(&self) -> bool {
        self.check
    }

    fn pretty_name(&self) -> String {
        "CreditChain CLI".to_string()
    }
    ...
    fn get_update_info(&self) -> Result<UpdateRequiredInfo> {
        ...
        let latest_release = loop {
            ...
            if release.version.starts_with("creditchain-cli-") {
                break release;
            }
        };
    ...
    fn build_updater(&self, info: &UpdateRequiredInfo) -> Result<Box<dyn ReleaseUpdate>> {
        ...
        Update::configure()
            .repo_owner(&self.repo_owner)
            .repo_name(&self.repo_name)
            .bin_name("creditchain")
            .current_version(current_version)
            .target_version_tag(&format!("creditchain-cli-v{}", info.target_version))
            .target(target)
...
#[async_trait]
impl CliCommand<String> for CreditChainUpdateTool {
