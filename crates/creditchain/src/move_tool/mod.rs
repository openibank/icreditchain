...
use creditchain_gas_schedule::{MiscGasParameters, NativeGasParameters};
use creditchain_move_debugger::creditchain_debugger::CreditChainDebugger;
use creditchain_rest_client::{
    creditchain_api_types::{EntryFunctionId, HexEncodedBytes, IdentifierWrapper, MoveModuleId},
    error::RestError,
    CreditChainBaseUrl, Client,
};
use creditchain_types::{
    account_address::{create_resource_address, AccountAddress},
    object_address::create_object_code_deployment_address,
    on_chain_config::creditchain_test_feature_flags_genesis,
    transaction::{
        ReplayProtector, Transaction, TransactionArgument, TransactionPayload, TransactionStatus,
    },
};
use creditchain_vm::data_cache::AsMoveResolver;
...
pub mod creditchain_debug_natives;
mod bytecode;
pub mod coverage;
mod fmt;
mod lint;
...
const HELLO_BLOCKCHAIN_EXAMPLE: &str = include_str!(
    "../../../../creditchain-move/move-examples/hello_blockchain/sources/hello_blockchain.move"
);
...
pub struct FrameworkPackageArgs {
    /// Git revision or branch for the CreditChain framework
    ///
    ...
    /// Local framework directory for the CreditChain framework
    ///
...
impl FrameworkPackageArgs {
    pub fn init_move_dir(
        ...
        if let Some(ref path) = self.framework_local_dir {
            dependencies.insert(CREDITCHAIN_FRAMEWORK.to_string(), Dependency {
            ...
            dependencies.insert(CREDITCHAIN_FRAMEWORK.to_string(), Dependency {
                local: None,
                git: Some(CREDITCHAIN_GIT_PATH.to_string()),
...
/// This can then be run with `libra2 move run-script`
#[derive(Parser)]
...
pub struct TestPackage {
    ...
    /// Collect coverage information for later use with the various `libra2 move coverage` subcommands
    #[clap(long = "coverage")]
...
impl CliCommand<&'static str> for TestPackage {
    ...
    async fn execute(self) -> CliTypedResult<&'static str> {
        ...
        let result = move_cli::base::test::run_move_unit_tests(
            ...
            creditchain_debug_natives::creditchain_debug_natives(
                NativeGasParameters::zeros(),
                MiscGasParameters::zeros(),
            ),
            creditchain_test_feature_flags_genesis(),
            None,
            None,
        ...
        if self.compute_coverage {
            ...
            println!("Please use `libra2 move coverage -h` for more detailed source or bytecode test coverage of this package");
        }
...
/// Publishes the modules in a Move package to the CreditChain blockchain
#[derive(Parser)]
...
fn create_package_publication_data(
    ...
    let payload = match publish_type {
        PublishType::AccountDeploy => {
            creditchain_cached_packages::creditchain_stdlib::code_publish_package_txn(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectDeploy => {
            creditchain_cached_packages::creditchain_stdlib::object_code_deployment_publish(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectUpgrade => {
            creditchain_cached_packages::creditchain_stdlib::object_code_deployment_upgrade(
                metadata_serialized.clone(),
                compiled_units.clone(),
...
/// Publishes the modules in a Move package to the CreditChain blockchain, under an object (legacy version of `deploy-object`)
#[derive(Parser)]
...
/// Publishes the modules in a Move package to the CreditChain blockchain, under an object.
#[derive(Parser)]
...
async fn submit_chunked_publish_transactions(
    ...
    if !is_staging_area_empty(txn_options, large_packages_module_address).await? {
        ...
        \nUse the `libra2 move clear-staging-area` command to clean up the `StagingArea` resource under the account.",
    ...
    for (idx, payload) in payloads.into_iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                \nTo avoid this, use the `libra2 move clear-staging-area` command to clean up the `StagingArea` resource under your account before retrying.".bold());
                return Err(e);
            },
        }
    }
...
async fn is_staging_area_empty(
    ...
    match staging_area_response {
        ...
        Err(RestError::Api(creditchain_error_response))
            if creditchain_error_response.error.error_code == CreditChainErrorCode::ResourceNotFound =>
        {
...
/// Publishes the modules in a Move package to the CreditChain blockchain under a resource account
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for CreateResourceAccountAndPublishPackage {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --profile or run libra2 init".to_string(),
            ));
        };
        ...
        let payload = creditchain_cached_packages::creditchain_stdlib::resource_account_create_resource_account_and_publish_package(
            seed,
...
impl CliCommand<TransactionSummary> for Replay {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        let rest_endpoint = match &self.network {
            Mainnet => "https://fullnode.mainnet.creditchain.io",
            Testnet => "https://fullnode.testnet.creditchain.io",
            Devnet => "https://fullnode.devnet.creditchain.io",
            RestEndpoint(url) => url,
        };

        // Build the client
        let client = Client::builder(CreditChainBaseUrl::Custom(
        ...
        let debugger = CreditChainDebugger::rest_client(client)?;
...
/// Duplicated from creditchain_types, as we also need to load_account_arg from the CLI.
#[derive(Debug, Clone)]
pub struct MemberId {
