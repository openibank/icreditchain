...
use creditchain_db_indexer::db_indexer::InternalIndexerDB;
...
use creditchain_jellyfish_merkle::iterator::JellyfishMerkleIterator;
use creditchain_logger::info;
use creditchain_metrics_core::TimerHelper;
use creditchain_schemadb::batch::{NativeBatch, SchemaBatch, WriteBatch};
...
use creditchain_storage_interface::{
    ...
    CreditChainDbError, DbReader, Result, StateSnapshotReceiver,
};
...
impl StateDb {
    fn expect_value_by_version(
        ...
        self.get_state_value_by_version(state_key, version)
            .and_then(|opt| {
                opt.ok_or_else(|| {
                    CreditChainDbError::NotFound(format!(
...
impl StateStore {
    ...
    // TODO: change to async comment once it does like https://github.com/ibankio/creditchain/blob/159b00f3d53e4327523052c1b99dd9889bf13b03/storage/backup/backup-cli/src/backup_types/state_snapshot/restore.rs#L147 or overlap at least two chunks.
    ...
    pub fn get_all_jmt_nodes_referenced(
        &self,
        version: Version,
    ) -> Result<Vec<creditchain_jellyfish_merkle::node_type::NodeKey>> {
        creditchain_jellyfish_merkle::JellyfishMerkleTree::new(self.state_merkle_db.as_ref())
            .get_all_nodes_referenced(version)
    }

    #[cfg(test)]
    pub fn get_all_jmt_nodes(&self) -> Result<Vec<creditchain_jellyfish_merkle::node_type::NodeKey>> {
        let mut iter = self
            .state_db
            .state_merkle_db
            .metadata_db()
        ...
        let mut keys: Vec<creditchain_jellyfish_merkle::node_type::NodeKey> =
...
mod test_only {
    ...
    use creditchain_schemadb::batch::SchemaBatch;
